...
/** Emitted widths per slot family. The hero is full-bleed, so it earns a third. */
const WIDTHS = { site: [600, 1200, 2000], default: [600, 1200] };
...
function ratioFor(slot) {
    if (slot.startsWith('site/')) return 21 / 9;
    if (slot.endsWith('-hero')) return 15 / 8;
    return 3 / 2;
}

function widthsFor(slot) {
    return slot.startsWith('site/') ? WIDTHS.site : WIDTHS.default;
}
 ...
 * off rather than fail: a 429 halfway through leaves a half-populated public/img
...
async function emit(slot, buffer) {
    ...
    for (const width of widthsFor(slot)) {
...
for (const [slot, id] of slots) {
    const widest = widthsFor(slot).at(-1);
